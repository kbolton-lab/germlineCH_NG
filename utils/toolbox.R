...
do_glm_and_plot_or <- function(data = NULL, response = NULL, terms_for_formula = NULL,  terms_to_iterate = NULL, terms_to_plot = NULL, max_OR = NULL, plot_name = NULL, width = 10, height = 4, term_factor = F, factors_for_term = NULL){
...
saveTxt <- function(object,path) {
  write.table(object,path, col.names = T, row.names = F, quote = F, sep = "\t")
}

saveVec <- function(object,path) {
  write.table(object,path, col.names = F, row.names = F, quote = F, sep = "\t")
}

saveRds = function(object,path) {
  saveRDS(object, path, ascii = FALSE, version = NULL,compress = TRUE, refhook = NULL)
}
