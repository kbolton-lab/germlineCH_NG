...
get_protein_info <- function (gene) {
        ...
        canonical <- getBM(attributes=c("ensembl_transcript_id","transcript_is_canonical","transcript_mane_select"), filters = "hgnc_symbol", values = gene, mart= ensembl)
        ...
        # Keep canonical
        canonical_id <- canonical[canonical$transcript_mane_select!="",1]
        cds.infos <- cds.infos[cds.infos$ensembl_transcript_id==canonical_id,]
        ...
        if (nrow(cds.infos)!=0)
        {
                ...
                residueSeq <- Biostrings::translate(DNAStringSet(codingSeq))
                # Filter out non AA codes
                residueSeq <- filter_residue(residueSeq)
                # Get length
                protein_length <- width(residueSeq)
                # Recover domains
                ...
                if(all(is.na(protein_domain$interpro_description))) {
                        protein_domain <- data.frame(interpro_description=gene, interpro_start=1, interpro_end=protein_length)
                }
        } else
        {
                return(NULL)
