...
my_lollipop <- function(dat.genetics.unique, current.gene, disease, protein_domain, protein_length, cutoff.hotspot=20, colors.mutation_type) {
    ...
    p.protein <- ggplot(protein_domain %>% arrange(width)) + 
            ...
            guides(fill=guide_legend(nrow=ifelse(colourCount < 3, 1, (colourCount %/% 2)+1))) +
    ...
    total_patients <- length(unique(dat.genetics.unique$sample_id))
    dat.score = dat.genetics.unique.gene %>%
        mutate(hit=get(disease)) %>%
        group_by(hit, PROTEIN_CHANGE, mutation_type) %>%
        summarise(
            gene = unique(gene),
            n_patients = length(unique(sample_id)),
